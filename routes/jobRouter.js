...
const { addJob, modify, getAll, getOne, del, saveJob, getUserSavedJobs, delUserSavedJob} = require("../controller/jobsController");
...
JobRouter.delete("/savedJobs/delone/:jobId/:userId", async(req,res) => {
    try {
        if(!req.isAuth ) throw new Error("Unauthenticated");
        const data = await delUserSavedJob(req);
        res.send(data);
    } catch (error) {
        res.send({Err : error.message})
    }
